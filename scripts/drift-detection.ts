...
function parseSpecFile(specPath: string): SpecSummary {
  ...
  for (const page of userPages) {
    ...
    try {
      ...
      findings.push({
        ...
        recommendation: 'Create user interface page for this story',
      });
    }
  }
  ...
  for (const page of adminPages) {
    ...
    try {
      ...
      findings.push({
        ...
        specReference: 'specs/001-headless-financial-api/spec.md',
        fileReferences: [`frontend/src/app/${page.path} (missing)`],
        recommendation: 'Create admin interface page for this story',
      });
    }
  }
...
function checkCrossCuttingConcerns() {
  ...
  try {
    ...
    findings.push({
      ...
      severity: 'high',
      title: 'Idempotency repository interface not found',
      description: 'Idempotency support interface not found',
  ...
  try {
    ...
    findings.push({
      ...
      severity: 'critical',
      title: 'Outbox repository interface not found',
      description: 'Transactional outbox pattern interface not found',
